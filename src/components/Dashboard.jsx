    ...
    <div className="container mx-auto p-4 w-full">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map(blog => (
            <div key={blog.id} className="bg-gray-100 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex flex-col">
              <h2 className="text-xl font-bold mb-2">{blog.title}</h2>
              <p className="mb-4 text-gray-700">{blog.description}</p>
              <img src={blog.imageUrl} alt={blog.title} className="mb-4 w-full h-48 object-cover rounded-lg" />
